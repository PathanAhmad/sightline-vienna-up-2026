...
def _process_lot_upload(uploaded_file: Any) -> None:
    """Handle Step 1's uploader: a single operator lot zip.

    Accepts the as-shipped operator zip (the one that contains the three
    ...
    if not lot_bundle.is_lot_bundle(name, raw):
        st.error(
            f"{name} doesn't look like an operator lot zip "
...
def render(geom_handle: dict | None) -> None:
    ...
    with st.container(border=True, key="card_rail_lot_upload"):
        st.markdown(
            ...
            "<span class='step-title'>Load operator lot</span>"
            "</div>"
            "<div class='upload-rail-help'>"
            "The CLP&hellip;.zip the operator ships with the three trench &amp; "
    ...
    with st.container(border=True, key="card_rail_upload"):
        st.markdown(
            ...
            "the seven Sightline checks &mdash; usually 6 s per photo. Segments "
